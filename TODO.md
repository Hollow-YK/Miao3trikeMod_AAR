...
14) 宏起始延迟增强（确保首个点击生效）
    - ✅ 拉长“删除录制 overlay → 开始宏（点击按钮中心）”的间隔（当前 1000ms），避免系统窗口切换/overlay 移除导致的首个点击丢失。

15) 宏启动加速（方案2：先隐藏/不可触摸，再开跑）
    - ✅ 录制结束后不立即 remove overlay；先将 overlay 设为不可触摸并隐藏（视觉消失 + 不拦截输入），然后尽快开始宏（当前等待 30ms）；宏结束时再统一清理。
